...
def set_params(train_prefix, model, identity_dim, epochs=10, dim1=128, dim2=128):
    #core params..
    flags.DEFINE_string('model', model, 'model names. See README for possible values.')  
    flags.DEFINE_float('learning_rate', 0.00001, 'initial learning rate.')
    flags.DEFINE_string("model_size", "small", "Can be big or small; model specific def'ns")
    flags.DEFINE_string('train_prefix', train_prefix, 'name of the object file that stores the training data. must be specified.')
    flags.DEFINE_integer('identity_dim', identity_dim, 'Set to positive value to use identity embedding features of that dimension. Default 0.')
    flags.DEFINE_integer('epochs', epochs, 'number of epochs to train.')
    flags.DEFINE_integer('dim_1', dim1, 'Size of output dim (final is 2x this, if using concat)')
    flags.DEFINE_integer('dim_2', dim2, 'Size of output dim (final is 2x this, if using concat)')




def log_dir():
    log_dir = FLAGS.base_log_dir + "/" + FLAGS.train_prefix.split("/")[0]
